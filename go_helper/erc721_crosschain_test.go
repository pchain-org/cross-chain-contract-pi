...
import (
	...
	game721 "github.com/pchain-org/cross-chain-contract-pi/go_abi/game721_abi"
	erc721 "github.com/pchain-org/cross-chain-contract-pi/go_abi/merc721_abi"
...
func TestERC721DepositAndWithdraw(t *testing.T) {
	...
	contractAddress, err := DeployPBridgeContract(chainID, rpcClient, privkHex, hexAddress)
	if err != nil {
	...
	for _, add := range []*tool.AddrInfo{a2, a3} {
		s, err := DynamicMultiSignNFTTx(randomTxKey[2:], add.PrivkHex, tokenURI, game721Address, destination, tokenId, version)
...
func TestMinerBurnERC721(t *testing.T) {
	...
	// multi sign
	for _, add := range []*tool.AddrInfo{a2, a3} {
		s, err := DynamicMultiSignNFTTx(randomTxKey[2:], add.PrivkHex, tokenURI, ERC721Address, destination, tokenId, version)
		tool.FailOnErr(t, err, "create sig failed")
		signs = append(signs, s...)
	}
	params := &TxDynamicParams{
	...
	tool.FailOnErr(t, err, "get NFT balance failed")
	fmt.Println("NFT balance of multsigBal:", multsigBal.String())
}
